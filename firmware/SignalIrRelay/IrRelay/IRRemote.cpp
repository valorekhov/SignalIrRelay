...
void IRrecv::enableIRIn() {
  ...
  // set pin modes
  //BIT(D,irparams.recvpin, DDR) = 1;
  if (irparams.recvpin < 8)
	sbi(PORTD, irparams.recvpin);
	else
	sbi(PORTB, irparams.recvpin);
...
ISR(TIMER_INTR_NAME)
{
  ...
  uint8_t irdata = (uint8_t)(irparams.recvpin < 8 ? PORTD & _BV(irparams.recvpin) : PORTB & _BV(irparams.recvpin));
